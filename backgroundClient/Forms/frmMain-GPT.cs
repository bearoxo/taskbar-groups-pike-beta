        ...
        */
        ...
        public frmMain(Category category, string[] arguments)
        {
            ...
            */
            ...
            // Add MouseClick event handlers for each ucShortcut control
            foreach (ucShortcut shortcutControl in ControlList)
            {
                shortcutControl.MouseClick += ucShortcut_MouseClick;
            }
        }

        // Event handler for MouseClick on ucShortcut controls
        private void ucShortcut_MouseClick(object sender, MouseEventArgs e)
        {
            // Check if it's a right-click event
            if (e.Button == MouseButtons.Right && sender is ucShortcut shortcutControl)
            {
                // Call ucShortcut_RightClick method to open the context menu for the right-clicked ucShortcut control
                shortcutControl.ucShortcut_RightClick(sender, e);
            }
        }
